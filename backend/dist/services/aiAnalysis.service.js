...
class AIAnalysisService {
    constructor() {
        ...
        this.geminiDisabledUntil = 0;
    }
    maybeAttachSingle(result, input) {
        try {
    ...
    disableGeminiTemporarily(minutes = 5) {
        this.geminiDisabledUntil = Date.now() + minutes * 60 * 1000;
    }
    buildProjectFailAdvisory(input, languageStyle) {
    ...
    async generateScopedAdvisory(input) {
        ...
        if (intent === 'forecast') {
            const forecastData = await weather_service_1.weatherService.getForecastOutlook(7);
    ...
    async generateEnsembleAdvisory(input, scopedQuery, languageStyle, variationSeed, preferredProvider) {
    ...
    async generateGeminiAdvisory(input, scopedQuery, languageStyle, variationSeed, skipAudit = false) {
    ...
    mergeAdvisoryResults(pythonResult, geminiResult, input, preferredProvider) {
        ...
        const confidenceScore = Math.min(0.99, Math.max(preferred.confidenceScore ?? 0, secondary.confidenceScore ?? 0, 0.82) + 0.03);
        ...
        const rationale = this.mergeStringLists(preferred.decisionBasis.rationale, secondary.decisionBasis.rationale, 5);
    ...
    mergeScopeNotes(primary, secondary) {
        return this.mergeStringLists([primary ?? ''], [secondary ?? ''], 2).filter(Boolean).join(' ');
    }
    mergeStringLists(primary = [], secondary = [], limit = 5) {
        const seen = new Set();
        const merged = [];
    ...
    mergeHealthDetails(primary, secondary) {
    ...
    pickHigherRiskLevel(left, right, weatherLevel) {
        const order = ['safe', 'caution', 'extreme-caution', 'danger', 'extreme-danger'];
        const candidates = [left, right, weatherLevel].filter(Boolean);
    ...
    pickHigherTriage(left, right) {
        const rank = { urgent: 3, high: 2, monitor: 1, low: 0 };
    ...
    async generatePythonAdvisory(input, scopedQuery, languageStyle, variationSeed, skipAudit = false) {
    ...
    varyText(seed, text, languageStyle) {
    ...
    buildForecastAdvisory(forecastDays, languageStyle, mode) {
        if (forecastDays.length === 0) {
            return this.buildFallbackAdvisory({ query: '', weather: { source: 'fallback', location: '', temperatureC: 0, humidityPercent: 0, condition: '', windSpeedMps: 0, pressureHpa: 0, heatIndexC: 0, heatLevel: 'safe', timestamp: new Date().toISOString() } }, 'Provide current school heat safety advisory based on the given weather.', languageStyle);
        }
